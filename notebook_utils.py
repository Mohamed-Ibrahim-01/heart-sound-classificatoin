...
def _get_random_samples(n_samples, seed):
    ...
    normal_samples = [
        (normal_physio[a+i][0], f"Noraml Sample {i}") for i in range(n_samples)]
    abnormal_samples = [
        (abnormal_physio[a+i][0], f"Abnoraml Sample {i}") for i in range(n_samples)]
...
def view_downsampled_dist():
    ...
    normal_cnt, abnormal_cnt = tuple(
        hs_df["Label"].value_counts(normalize=True))
    axs[1].pie([normal_cnt, abnormal_cnt], labels=[
               "Normal", "Abnormal"], autopct='%.0f%%', textprops={'fontsize': 15})
    axs[0].axis('off')
    axs[2].axis('off')
    plt.show()
...
def view_seconds_dist():
    ...
    axs[1].pie([normal_length, abnormal_length], labels=["Normal", "Abnormal"], autopct='%.0f%%', textprops={'fontsize': 15}
               )
    ...
    if grid_search:
        if clf['name'] in set(grid_search):
            ...
            classifier = GridSearchCV(
                clf['method'](), clf['parameters'], cv=5, scoring="f1_macro")
...
def confusion_compare(fitted_clfs, data):
    ...
    for cls, ax in zip(fitted_clfs, axes.flatten()):
        ConfusionMatrixDisplay.from_estimator(
            cls[1], inputs, targets, ax=ax, colorbar=False)
